...
from bwb.utils.validation import shape_validation
...
class DistributionP(t.Protocol):
    """
    Protocol for the discrete distributions, like the classes ``DiscreteDistribution`` and
    ``DistributionDraw``.
    """
...
class DiscreteDistribution(torch.distributions.Categorical):
    """
    Class that represent a discrete distribution, using de ``Categorical`` class provided by
    PyTorch.
    """
...
class DistributionDraw(DiscreteDistribution):
    ...
    def __init__(
        ...
        """
        # Probably this is useless, because the super class already does this
    ...
    def from_array(
        ...
        grayscale: torch.Tensor = torch.as_tensor(
            grayscale, dtype=torch.uint8, device=device  # Use uint8 for images
        ).squeeze()
    ...
    def grayscale(self) -> torch.Tensor:
        ...
        if self._grayscale_weights is not None:
            ...
            self._grayscale = grayscale_parser(
                self.shape, weights, support, dtype=self.dtype, device=self.device
            )
