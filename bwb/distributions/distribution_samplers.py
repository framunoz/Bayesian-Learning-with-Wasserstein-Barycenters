...
class DiscreteDistribSampler[DistributionT](DistributionSampler[DistributionT]):
    ...
    def _rvs(self, size: int = 1, seed: seed_t = None) -> tuple[t.Sequence[DistributionT], list[int]]:
        """Samples as many distributions as the ``size`` parameter indicates."""
        rng: torch.Generator = set_generator(seed=seed, device=config.device)
...
class BaseGeneratorDistribSampler[DistributionT](ContinuousDistribSampler[DistributionT], metaclass=abc.ABCMeta):
    ...
    def _get_noise(self, size: int = 1, seed: seed_t = None) -> torch.Tensor:
        """
        Get noise samples. If the seed is None, a random seed will be used. If the noise sampler fails to generate
        noise with the seed, a warning will be raised and the noise will be generated without the seed.
        ...
        try:
            ...
            msg = f"Failed to generate noise with seed. The noise will be generated without the seed. {e}"
            _log.warning(msg)
...
class GeneratorDistribSampler(BaseGeneratorDistribSampler[dist.DistributionDraw]):
    ...
    def _rvs(self, size: int = 1, seed: seed_t = None) -> tuple[t.Sequence[dist.DistributionDraw], torch.Tensor]:
        noises: torch.Tensor = self._get_noise(size, seed)
...
def __main():
    input_size = 128
    output_size = (32, 32)
    z_ = torch.rand((1, input_size, 1, 1)).to(config.device)
