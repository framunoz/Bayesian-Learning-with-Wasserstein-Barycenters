...
from bwb.utils.protocols import device_t, DiscreteDistribSamplerP, DrawP, seed_t
...
def normalised_samples_ordered_dict(posterior: DiscreteDistribSamplerP):
    """
    Function that returns an ordered dictionary with the normalised samples in the posterior
    instance.
...
def likelihood_ordered_dict(posterior):
    """
    Function that returns an ordered dictionary with the likelihoods of the samples in the
    posterior.
...
def normalised_steps_ordered_dict(mcmc):
    ...
    """
    msg = ("The normalised_steps_ordered_dict function is deprecated and will be removed in "
           "the future.")
    _log.warning(msg)
    warnings.warn(msg, DeprecationWarning, stacklevel=2)
